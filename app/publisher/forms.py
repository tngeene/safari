...
from wtforms.fields import BooleanField, StringField, SubmitField, FloatField, PasswordField, IntegerField, DateField, \
    SelectField, FieldList, TextAreaField, FormField, HiddenField, DecimalField, FloatField, SelectMultipleField
...
class EditPriceForm(Form):
    ...
    price_per_day_children = DecimalField(validators=[DataRequired()])
    price_per_day_adults = DecimalField(validators=[DataRequired()])
    includes = FieldList(FormField(IncludesForm, default=lambda: Include()), min_entries=1)
...
class EditProfileForm(Form):
    ...
    locations = FieldList(FormField(LocationForm, default=lambda: Publocations()), min_entries=1)
    paypal = StringField('paypal', validators=[Length(min=2, max=80), Email()])
    facebook = StringField(validators=[Length(min=0, max=100)])
    twitter = StringField(validators=[Length(min=0, max=100)])
    instagram = StringField(validators=[Length(min=0, max=100)])
