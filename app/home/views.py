...
def listings():
    ...
    listings = Listing.query.filter(Listing.status == True).order_by(Listing.rating.desc()).paginate(page,current_app.config['POSTS_PER_PAGE'],False)
    ...
    arcodes=get_arcode()
    return render_template('home/list.html',listings=listings.items, next_url=next_url, prev_url=prev_url,arcodes=arcodes,categories=categories,maxval=maxval)
...
def booking(id):
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            payment = Payment(booking=booking, user_id=current_user.id, years=year, month=month, billing_zip_code=billing_code,
...
def tour_operators_list():
    page = request.args.get('page', 1, type=int)
    publishers = Publisher.query.order_by(Publisher.overal_ratings.desc()).paginate(page,current_app.config['POSTS_PER_PAGE'],False)
    ...
    arcodes=get_arcode()
    return render_template('home/tour_operators.html', publishers=publishers.items,arcodes=arcodes,offices=offices,next_url=next_url, prev_url=prev_url)
...
def tour_operator_listings(publisher_id):
    page = request.args.get('page', 1, type=int)
    publisher = User.query.filter_by(id=publisher_id).first()
    publisher_listings = Listing.query.filter_by(publisher_id=publisher_id).filter(Listing.status == True).order_by(Listing.createdAt.desc()).paginate(page,8,False)
    next_url = url_for('home.tour_operator_listings',publisher_id=publisher_id, page=publisher_listings.next_num) if publisher_listings.has_next else None
    prev_url = url_for('home.tour_operator_listings',publisher_id=publisher_id, page=publisher_listings.prev_num) if publisher_listings.has_prev else None
    arcodes=get_arcode()
    return render_template('home/operator_listings.html', publisher=publisher,
        publisher_listings=publisher_listings.items, next_url=next_url, prev_url=prev_url,arcodes=arcodes)
...
def _get_category():
    ...
    prev_url = url_for('home.listings', page=listings.prev_num) if listings.has_prev else None
    arcodes=get_arcode()
    return render_template('home/_listings.html',listings=listings.items,arcodes=arcodes,next_url=next_url,prev_url=prev_url)
...
def _get_office():
    ...
    prev_url = url_for('home.tour_operators', page=publishers.prev_num) if publishers.has_prev else None
    arcodes=get_arcode()
    return render_template('home/_tour_operators.html',publishers=publishers.items,arcodes=arcodes,next_url=next_url,prev_url=prev_url)
