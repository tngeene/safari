    ...
    if Booking.query.count() > 3:
        popular_list = [
            ...
            for row in db.session.query(
                Listing.id, func.count(Booking.listing_id).label("total")
            )
            .join(Booking)
            .group_by(Listing)
            .order_by("total DESC")
        ...
        popular_packages = (
            Listing.query.filter(Listing.id.in_(popular_list))
            .filter(Listing.status == True)
            .limit(6)
        )
    popular_operators = (
        Publisher.query.join(User, (User.id == Publisher.user_id))
        .filter(User.status == True)
        .order_by(Publisher.overal_ratings.desc())
        .limit(3)
    )
...
def listings():
    ...
    listings = (
        Listing.query.filter(Listing.status == True)
    ...
    next_url = (
        url_for("home.listings", page=listings.next_num) if listings.has_next else None
    )
    prev_url = (
        url_for("home.listings", page=listings.prev_num) if listings.has_prev else None
    )
    maxlist = Listing.query.filter(Listing.status == True).all()
    maxval = (
        int(max(node.price.total_price_adults for node in maxlist)) if maxlist else 0
    )
    arcodes = get_arcode()
    return render_template(
        "home/list.html",
...
def listings_by_details(id):
    ...
    form = BookingForm()
    form.total.data = (
        listings.price.total_price_children + listings.price.total_price_adults
    )
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            payment = Payment(
                ...
                card_identification_number=email,
...
def tour_operators_list():
    ...
    publishers = (
        Publisher.query.join(User, (User.id == Publisher.user_id))
        .join(Publocations, (Publocations.publisher_id == Publisher.id))
        .filter(User.status == True)
        .filter(Publocations.country.like("%" + place + "%"))
        .order_by(Publisher.overal_ratings.desc())
        .paginate(page, current_app.config["POSTS_PER_PAGE"], False)
    )
    next_url = (
        url_for("home.tour_operators", page=publishers.next_num)
        if publishers.has_next
        else None
    )
    prev_url = (
        url_for("home.tour_operators", page=publishers.prev_num)
...
def tour_operator_listings(publisher_id):
    ...
    publisher = User.query.filter_by(id=publisher_id).first()
    publisher_listings = (
        Listing.query.filter_by(publisher_id=publisher_id)
        .filter(Listing.status == True)
    ...
    next_url = (
        ...
        if publisher_listings.has_next
        else None
    )
    prev_url = (
        url_for(
        ...
        if publisher_listings.has_prev
        else None
    )
    arcodes = get_arcode()
    return render_template(
...
def tour_operator_reviews(publisher_id):
    publisher = User.query.filter_by(id=publisher_id).first()
    publisher_listings = Listing.query.filter_by(publisher_id=publisher_id).filter(
        Listing.status == True
    )
    arcodes = get_arcode()
    return render_template(
...
def tour_operator_destinations(publisher_id):
    publisher = User.query.filter_by(id=publisher_id).first()
    publisher_listings = Listing.query.filter_by(publisher_id=publisher_id).group_by(
        Listing.location
    )
    arcodes = get_arcode()
    return render_template(
...
def countries_and_parks_list():
    ...
    arcodes = get_arcode()
    return render_template(
        "home/countries_and_parks.html", countries=countries, arcodes=arcodes
    )
...
def disclaimer():
    ...
    return render_template("home/disclaimer.html", disclaimer=disclaimer)
